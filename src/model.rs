...
impl SqliteModel {
    ...
    pub fn rollback(&self) -> Result<(), ()> {
        self.connection.execute("ROLLBACK;").map_err(log_and_ignore)
    }
...
impl Model for SqliteModel {
    ...
    fn add_document(&mut self, path: PathBuf, content: &[char]) -> Result<(), ()> {
        ...
        let mut insert = self.connection.prepare(query).map_err(|err| {
            eprintln!("ERROR: could not prepare statement for inserting document: {err}");
        })?;
        ...
        insert
            .bind((":term_count", Lexer::new(content).count() as i64))
            .map_err(log_and_ignore)?;
